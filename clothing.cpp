...
void printMatrix(int **inputmatrix, int rowSize, int colSize, char ** rowNames, char **colNames, char * colsuffix)
{
  cout << "\n\n\t-----  Printing Matrix    ----\t\n\n";
  cout << "\t";
  for (int j = 0; j < colSize; j++) {
      cout << colNames[j] << "-"<<colsuffix[j] << "\t";
    }
  cout << "WinterSum"<<"\t";
  cout << "SummerSum"<<"\t";
  for (int i = 0; i < rowSize; i++) {
    ...
    for (int j = 0; j < colSize; j++) {
      ...
      if (colsuffix[j] == 'w'){
        wintersum += inputmatrix[i][j];
      }
      else{
        summersum += inputmatrix[i][j];
      }
    }
    cout << wintersum << "\t\t";
    cout << summersum << "\t";
...
*@return distrbution summation of type float
*/
Solution createSolutionFromPath(int **matrixSolutionPath, int numChild, int numClothes, float rByNRatio, char * seasons){
  // Check if path satisfies summer and winter constraint
  for (int i =0; i < numChild; i++){
    bool atleastWinter = false;
    bool atleastSummer = false;
    for (int j=0; j < numClothes; j++){
      if (matrixSolutionPath[i][j] > 0){
        if (seasons[j] == 'w'){
          atleastWinter = true;
        }
        if (seasons[j] == 's'){
    ...
    if ((atleastWinter == false) || (atleastSummer == false)){
...
Solution minFairPriceRecursionFn(int ** currPriceMatrix, int numChild, int numClothes, int currIx, int currJx,
                                        int ** currMatrixPath, float rByNRatio, Solution currGlobalOptimum, char *seasons) {
    ...
    { // explore to the right columns i.e. children
      ...
      Solution minTopBottomSolution = minimumFairPriceBetweenTwo(
        minFairPriceRecursionFn(currPriceMatrix, numChild, numClothes, currIx + idx,
          currJx + 1, copiedNewMatrixPath, rByNRatio, currGlobalOptimum, seasons),
        minFairPriceRecursionFn(currPriceMatrix, numChild, numClothes, currIx - idx - 1, currJx + 1,
          copiedNewMatrixPath, rByNRatio, currGlobalOptimum, seasons)
      );
  ...
  { //Full path explored
    return createSolutionFromPath(currMatrixPath, numChild, numClothes, rByNRatio, seasons);
  }
}
...
*/
Solution minFairPriceDifference(int ** priceMatrix, int numChild, int numClothes, float rByNRatio, char * clothSeasons)
{
...
int main(int argc, char ** argv) {
  ...
  ************************************************************************************************
  ...
  Documented and structured and easily readible code. All test cases passing

  References:
  1. Dean, B. C. (2004). Algorithms for minimum‐cost paths in time‐dependent networks with waiting policies. Networks: An International Journal, 44(1), 41-46.
  2. https://www.educative.io/answers/how-to-find-minimum-cost-path-in-all-pairs-shortest-path-problem  
  ************************************************************************************************
  ***********************************************************************************************/
  ...
  cout << "\nPrinting initial price matrix with all eligible mappings:\n";
  printMatrix(priceMatrix,numChild,numClothes,childIDs,clothesIDs, season);
  ...
  Solution optimalGlobalMinimumSolution = minFairPriceDifference(priceMatrix, numChild, numClothes, rByNRatio, season);
